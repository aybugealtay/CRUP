...
# load data file
data_matrix <- suppressMessages(readRDS(matrix))
...
# rds format
out.rds <- paste0(outdir, paste0(gsub(".data_matrix.rds","", basename(matrix)), ".rds"))
cat(paste0(skip(), "save predictions in rds format: ", out.rds))
saveRDS(data_matrix, gsub("/{1,}","/",out.rds))
done()

# bw format
out.bw <- paste0(outdir, paste0(gsub(".data_matrix.rds","", basename(matrix)), ".bw"))
cat(paste0(skip(), "save predictions in bw format: ", out.bw))
colnames(elementMetadata(data_matrix)) <- "score"
export(data_matrix, gsub("/{1,}","/",out.bw))
done()
...
if (length(peaks) > 0) {
  out.bedgraph <- paste0(outdir, paste0(gsub(".data_matrix.rds","", basename(matrix)), ".singleEnh.bedGraph"))
...
if (length(cluster) > 0) {
  out.bed <- paste0(outdir, paste0(gsub(".data_matrix.rds","", basename(matrix)), ".clusterEnh.bed"))
